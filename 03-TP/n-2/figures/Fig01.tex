...
\newcommand{\background}[5]{%
  \begin{pgfonlayer}{background}
    ...
    \path (#1.west |- #2.north)+(-1.0,1.0) node (a1) {};
    % Right-bottom corner of the background rectanle
    \path (#3.east |- #4.south)+(+1.0,-1.0) node (a2) {};
    % Draw the background
    \path[fill=yellow!20,rounded corners, draw=black!50, dashed]
      (a1) rectangle (a2);
    \path (a1.east |- a1.south)+(1.8,-0.8) node (u1)[texto]
      {\scriptsize\textsc{#5}};
  \end{pgfonlayer}}
...
\path \practica {1}{\ce{LiMn2O4}}{Section};
\path (p1.north)+(2.5,2.5) \practica{2}{\ce{MnCO3}}{Section};
\path (p1.north)+(2.5,5.0) \practica{4}{\ce{MnSO4}}{Section};
\path (p1.north)+(-2.5,5.0) \practica{3}{\ce{Li2CO3}}{Section};
...
\path [line] (p2.south) -- +(0.0,-0.5) -- +(-2.5,-0.5)
    -- node [below, midway] {} (p1) ;   
\path [line] (p4.south) -- +(0.0,+0.0) -- +(0.0,+0.0)
    -- node [above,midway] {} (p2) ; 
\path [line] (p3.south) -- +(0.0,-3.0) -- +(2.5,-3.0)
    -- node [above] {} (p1) ; 
%\path [line] (p3.north) -- node [above] {} (p6) ;
%\path [line] (p1.north) -- +(0.0,+1.0) -- +(+4.0,+1.0)
%    -- node [above, midway] {} (p5) ; 

\path (p3.south) -- +(0.0,-3.0) -- +(2.5,-3.5)
    -- node [above, midway] {\scriptsize ATG} (p1) ; 
\path (p3.south) -- +(5.0,-3.0) -- +(5.0,-1.8)
    -- node [above, midway] {\scriptsize DRX, IR} (p1) ; 
\path (p3.south) -- +(5.0,3.5) -- +(5.0,2.5)
    -- node [above, midway] {} (p1) ; 
\path (p1.south) -- +(0.0,-1.0) -- +(0.0,-1.0)
    -- node [above, midway] {\scriptsize DRX \& IR} (p1) ; 

%\path (p3.south) -- +(0.0,-3.0) -- +(0.0,-1.5)
%    -- node [above, midway] {\scriptsize DRX \& IR} (p1) ; 
%\path (p3.south) -- +(5.0,-3.0) -- +(5.0,-1.5)
%    -- node [above, midway] {\scriptsize ATG \& DSC} (p1) ; 
%\path (p3.south) -- +(5.0,3.5) -- +(5.0,2.5)
%    -- node [above, midway] {\scriptsize DRX \& IR} (p1) ; 
%\path (p1.south) -- +(0.0,-1.0) -- +(0.0,-1.0)
%    -- node [above, midway] {\scriptsize DRX \& IR} (p1) ; 

%\transreceptor{p2}{CW}{ur2}
  
%Background 
%\backgroundbis{p1}{p5}{p4}{p4}{Polymère}
\background{p3}{p4}{p4}{p1}{Synthèses \& Caractérisations}
