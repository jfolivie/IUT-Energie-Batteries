...
Il existe une analogie entre une batterie et certains systèmes mécaniques. Les stations de transfert d'énergie par pompage, représentées à la Fig. \ref{fig:Chp-0-15}, sont un bon exemple modèle. Lorsque le réseau électrique est en déficit d'énergie électrique, ces stations de haute montagne déversent l'eau du bassin supérieur vers le bassin inférieur. Cette phase correspond à la phase de "décharge" du bassin, où l'énergie potentielle de pesanteur est convertie en énergie électrique. Dans le cas où le réseau est excédentaire en énergie, l'eau du bassin est remontée par un système de pompe. La STEP convertit donc l'énergie électrique en énergie potentielle de pesanteur, et vice-versa.


Dans une batterie, un opérateur va mettre en contact deux électrodes\mySideNote{Conducteur électronique (\emph{par ex.} un métal) ou ionique (\emph{par ex.} le verre) captant ou libérant des électrons} où les électrons ont une énergie potentielle chimique, aussi appelée enthalpie libre, différente. Lors de la mise en contact, cette différence conduit à la mise en mouvement des électrons \mySideNote{Ce flux se manifeste par un courant électrique, d'intensité notée $I$, qui représente la quantité de charge transporté à travers un conducteur par unité de temps.}. Le flux d'électron conduit petit à petit à un retour à l'équilibre du système qui se traduit par une égalité de l'énergie potentielle.

Lors de ce processus, \blancbis{deux espèces chimiques redox}, c'est-à-dire des espèces chimiques capables d'\blancbis{échanger un ou plusieurs électrons}, réagissent ensemble. Cette réaction libère de l'énergie sous deux formes : 
\begin{itemize}
    \item un \blancbis{travail utile} $W'$ qui est le travail électrique reçu ;
    \item une \blancbis{déperdition d'énergie}, la chaleur $Q$, c'est-à-dire de l'énergie thermique reçue.
\end{itemize}
...
Cette aptitude d'une (ou des) cellule(s) électrochimique(s) permet de les classer.

\section{Classification des générateurs électrochimiques}

\begin{center}
\includegraphics[scale=1.0]{figures-Chp0/Fig10}
\end{center}
...
\centering
\begin{tabular}{cc}
\subfloat[\label{fig:03a}]{\includegraphics[width=3cm]{figures-Chp0/Fig03a}} & \subfloat[\label{fig:03c}]{\includegraphics[width=3cm]{figures-Chp0/Fig03c}} \\
\subfloat[\label{fig:03b}]{\includegraphics[width=3cm]{figures-Chp0/Fig03b}} & \subfloat[\label{fig:03d}]{\includegraphics[width=3cm]{figures-Chp0/Fig03d}}
\end{tabular}
...
Si la pile AAA, illustré Fig. \ref{fig:03a}/\ref{fig:03b}, peut-être rechargée par le générateur, on peut dire qu'elle est un \blancbis{accumulateur}. Sinon, elle est appelée une \blancbis{pile}.

\begin{figure}[H]
\begin{center}
\begin{minipage}{0.5\textwidth}
\centering
\subfloat[\label{fig:09a}]{\includegraphics[height=2.5cm]{figures-Chp0/Fig09a}} \hspace{0.5cm} \subfloat[\label{fig:09b}]{\includegraphics[height=2.5cm]{figures-Chp0/Fig09b}}
\end{minipage}\begin{minipage}{0.5\textwidth}
\captionof{figure}{\small Cellule électrochimique en \ref{fig:09a} convention générateur et \ref{fig:09b} convention récepteur.}
\end{minipage}
\end{center}
\end{figure}

Du point de vue électrique, l'accumulateur est un \blancbis{générateur électrique} lorsqu'il se décharge ; un \blancbis{récepteur électrique} lorsqu'il se charge. La pile est quant à elle uniquement un \blancbis{générateur électrique}.


\subsection{Dispositifs constitués de plusieurs cellules électrochimiques}

\blancbis{La batterie} est le nom donné à tout dispositif électrique résultant de la mise en \blancbis{série} et/ou en \blancbis{parallèle}\mySideNote{\`{A} vous de deviner pourquoi une batterie a pour symbole électrique : 
\begin{center}
...
\begin{enumerate}
    \item \blancbis{primaire} lorsque l'on a mis en série des piles ;
    \item \blancbis{secondaire} lorsqu'on l'on a mis en série des accumulateurs.
\end{enumerate}

Ainsi, tout générateur primaire ne peut être utilisé \blancbis{qu'une seule fois}, et doit être jeté ou recyclé après utilisation. 
...
Afin de modéliser le flux d'électron, il est possible de décomposer l'équation de réaction en deux demi-équations redox\mySideNote{\'{E}quation modélisant la transformation chimique au sein d'un couple redox} modélisant chacune une \blancbis{oxydation} et une \blancbis{réduction}.
...
\definition{Deux entités chimiques faisant partie d'une même équation redox forment un couple redox, noté le plus souvent par :
\begin{align*}
...
\begin{comment}
\exercice{Indiquer les demi-équation redox et si il s'agit de la modélisation d'une oxydation/réduction sachant que les couples redox mis en jeux sont les suivants :
\begin{enumerate}
...
\section{Description d'une cellule électrochimique}

Nous allons concentrer notre attention sur les composantes d'une cellule électrochimique, qui qualifiera de manière indifférenciée piles et accumulateurs.
...
La \blancbis{polarisation} électrique\mySideNote{Lorsque l'on parle de polarisation électrique, on sous-entend qu'il existe un pôle $\oplus$/$\ominus$ non confondu d'où le terme de dipôle.} d'une cellule électrochimique ou batterie est invariante, peu importe le rôle joué au sein d'un circuit électrique.

\`{A} la Fig. \ref{fig:03b}, la batterie joue le rôle de générateur, c'est elle qui va imposer le sens du courant, d'intensité $I$ (donc le sens des électrons). \`{A} la Fig. \ref{fig:03d}, la batterie joue le rôle de récepteur, c'est donc le générateur de tension qui impose la polarisation. Dans les deux cas, les pôles de la batterie sont inchangés\mySideNote{Dans un générateur (resp. récepteur) électrique, le courant sort (resp. entre) par le pôle $\oplus$}. 
...
\hline
\end{tabular}
{\small \captionof{table}{\small Relation entre électricité et chimie dans une cellule électrochimique ou une batterie.}}
\label{tab:Chp-0-conventions}
\end{center}
%Rôle joué par la cellule dans le circuit

\attention{
Par abus de langage, les chimistes portant peu d'intérêt au rôle de la batterie au sein d'un circuit électrique utilise le terme anode (resp. cathode) en parlant du pôle $\ominus$ (resp. $\oplus$) en prenant comme référence la cellule ou batterie en décharge.\\
...
\end{minipage}\hfill   
\begin{minipage}[c]{0.35\textwidth}     
{\small \captionof{figure}{\small Pile Daniell réalisée à l'aide :} \small
\begin{itemize}
...
\definition{
\blancbis{Solvant} ~  Il s'agit de l'espèce permettant de \blancbis{solvater} les espèces redox et électrolytiques.
...
\begin{enumerate}
    ...
    \item garantir le processus de migration au sein de la solution à la place des espèces électroactives.
\end{enumerate}

~

\blancbis{Séparateur} ~  En général, les cellules sont composées d'un séparateur qui permet de séparer \blancbis{physiquement} et \blancbis{spécifiquement} les espèces électroactives du compartiment anodique et cathodique. Le séparateur est soit une membrane, soit un pont salin. Ce séparateur obéit à trois contraintes : jouer le rôle d'isolant vis-à-vis des électrons, empêcher toute réaction parasite entre espèces électroactives, tout en assurant une bonne conductivité ionique d'espèces chimiques chargées, mais inertes chimiquement.
}
...
Dans les études que nous allons faire sur les batteries, nous chercherons avant tout à établir un lien structure/propriété entre la \emph{tension à courant nul} et la structure chimique aux électrodes. En effet, la tension à courant nul est associée à une grandeur thermodynamique, appelée force électromotrice et notée $e$\mySideNote{On la rencontrera en TP sous le terme OCV pour \textbf{Open-circuit voltage}}, qui quantifie la force électrique permettant de séparer les charges au sein de la batterie



\example{
Le voltmètre dispose d'une très grande résistance (idéal = résistance infinie), on va donc bien mesurer la tension à courant nul. De plus, si les électrodes sont des phases homogènes et isotropes macroscopiquement, le potentiel électrostatique est constant. On va pouvoir décomposer le potentiel électrique en fonction des différents points dans la batterie schématisée à la Fig. \ref{fig:Chp-0-06-phases}

\begin{center}
\begin{minipage}{0.5\textwidth}
\includegraphics[width=5cm]{figures-Chp0/Fig06}
\end{minipage}\begin{minipage}{0.5\textwidth}
\small \captionof{figure}{\small Représentation des phases mises en jeux au sein d'une batterie (l'électrolyte suppose l'existence éventuelle d'une phase électrolytique en contact avec l'interface, ainsi que d'un éventuel pont salin).}
...
\section{Type de réactions au sein des accumulateurs lithium}

On va chercher à classer les réactions des cellules électrochimiques utilisant du lithium en deux grandes catégories : 
\begin{enumerate}
\item les cellules qui conduisent à l'\blancbis{apparition} et la \blancbis{disparition} d'une phase contenant des ions lithiums ;
\item les cellules dans lesquelles les matériaux sont des "éponges" d'ions lithium et sont capables de les accueillir \blancbis{sans qu'il y ait apparition/disparition} d'une phase.
\end{enumerate}
...
\captionof{figure}{\small Cellule électrochimique de S. Whittimgham.}
\end{minipage}\end{center}
On voit que \ce{TiS2} est un matériau fonctionnant comme une "éponge" à ions lithium.

\begin{center}
\includegraphics[scale=1]{figures-Chp0/Fig19-tot}
\captionof{figure}{Type de réactions dans les batteries Li-ion.}
\end{center}
...
Il existe une autre catégorie de réaction dans les systèmes électrochimiques où une espèce chimique redox, accueillie au sein d'un matériau hôte stable migre, lors de la décharge, vers les sites interstitiels inoccupés d'un autre matériau stable.

%Image des deux cas de figure

En général, l'incorporation de l'espèce accueillie à lieu de manière \blancbis{topotactique}\mySideNote{\'{E}spèces accueillies s'établissent dans des sites spécifiques de la structure cristalline.} et non pas de manière \blancbis{atactique}\mySideNote{\'{E}spèces accueillies s'établissent dans des sites aléatoires de la structure cristalline.}.
...
Notons dès à présent que dans le cas des matériaux lamellaires\mySideNote{Voir Chp. 3}, les \blancbis{réactions} \blancbis{d'insertion} sont parfois appelées \blancbis{réactions d'intercalation}.
...
Les cellules électrochimiques sont des dispositifs électriques. Par conséquent, trois grandeurs électriques usuelles permettent de les caractériser : la charge (notée $Q$), la tension (notée $U$) et le courant (notée $I$). Le courant dérive de la charge\mySideNote{Il faut savoir que :
\begin{align*}
I = \frac{dQ}{dt}
\end{align*}
}.
...
Dans le domaine des énergies, il est courant d'utiliser des wattheures [\si{\watt\hour}] à la place des joules [\si{\joule}] comme unité commerciale.

\attention{Un watt-heure correspond au nombre de joule reçu par un dispositif recevant une puissance de \SI{1}{\watt} pendant \SI{1}{\hour} :
\begin{align*}
1 ~ \si{\watt\hour} = 3600 ~ \si{\joule}
\end{align*}
}

\subsubsection{Capacité spécifique (ou nominale)}
...
En général, les chimistes préfèrent définir une \blancbis{capacité spécifique} massique ou volumique laquelle est \blancbis{intensive}.

\definition{
La capacité spécifique, notée $CS$, est définie comme : 
\begin{itemize}
\item le rapport de la capacité sur la masse du dispositif, d'où l’appellation de capacité spécifique \blancbis{massique} : 
\begin{align*}
CS_m = \frac{\text{Charge maximale stockable}}{\text{Masse du dispositif}}
\end{align*}
\item le rapport de la capacité sur le volume du dispositif, d'où l’appellation de capacité spécifique \blancbis{volumique} :
\begin{align*}
...
%WARNING def
\attention{
Un ampère-heure correspond au nombre d'ampère reçu par un dispositif recevant une intensité de \SI{1}{\ampere} pendant \SI{1}{\hour} :
\begin{align*}
1 ~ \si{\ampere\hour} = 3600 ~ \si{\coulomb}
\end{align*}

Surprenament, la capacité \blancbis{spécifique} sera liée à la nature chimique du matériau d'électrode et non à sa structure. Nous montretons que la capacité spécifique massique s'exprime comme : 
\begin{align*}
CS_m = \frac{z}{3.6 M} \quad [\si{\milli\ampere\hour\per\gram} = \si{\ampere\hour\per\kilo\gram}]
\end{align*}

avec $z$ est le nombre d'électrons délivrés par motif du matériau et $M$ la masse molaire du matériau hôte.
}

\subsubsection{\'{E}nergie théorique maximale (ETM)}
...
\definition{
L'énergie théorique maximale, notée ETM, se définit comme le travail électrique que l'on peut délivrer au maximum : 
\begin{equation}
ETM = \text{force électromotrice} \times \text{capacité spécifique massique}
\end{equation}
...
\subsection{\'{E}tat d'une cellule ou batterie pour un (ou plusieurs) cycle(s) : SoC, DoD et SoH}
...
\begin{equation*}
    SoC [\si{\percent}]= \frac{\text{charge emmagasinée par la batterie}}{\text{charge maximale que peut emmagasiner la batterie}} \times 100
    \label{eq:SoC}
\end{equation*}}
...
\begin{equation*}
    DoD [\si{\percent}]= \frac{\text{charge non emmagasinée par la batterie}}{\text{charge maximale que peut emmagasiner la batterie}} \times 100
    \label{eq:DoD}
\end{equation*}}

Si $DoD =$ \SI{0}{\percent} (resp. \SI{100}{\percent}), la batterie est chargée (resp. déchargée)
...
Par définition, $DoD [\si{\percent}] = SoH [\si{\percent}] - SoC [\si{\percent}]$. Si $DoD =$ \SI{0}{\percent} (resp. \SI{100}{\percent}), la batterie est chargée (resp. déchargée)
...
En général, du point de vu électrique, la \blancbis{qualité de l'énergie électrique} est supérieure lorsque la tension électrique est élevée. Ceci est simple à comprendre, si vous alimentez une résistance $R$ avec votre pile, de tension assimilée à la f.e.m. $e$, la puissance reçue par la résistance est : 
\begin{equation}
P = U_R \cdot I = \frac{e^2}{R}
\end{equation} 

ainsi, du fait du terme en $e^2$; la puissance électrique reçu par un tel dispositif est très sensible à la tension. On peut arbitrairement donner quelques plages de tension, associées aux applications courantes, permettant de définir la qualité de la tension électrique\mySideNote{Un chimiste ne cherchera pas forcément à optimiser la tension de sortie d'une cellule. En effet, pour certaines applications, comme les semi-conducteurs pour l'électronique, on cherchera en général à avoir de petites tensions (< \SI{1}{\volt}).} : 
\begin{center}
...
Cependant, il peut exister des applications allant au-delà de ces plages (\emph{par ex.} les voitures électriques nécessitent des tensions autour de \SI{200}{\volt}).
...
\input{\FigPath/Fig07.pgf}
\end{minipage} \hfill \begin{minipage}{0.25\textwidth}
\captionof{figure}{\small \'{E}volution de la capacité initiale pour différentes pertes par cycle de charge/décharge.}
\label{fig:Chp-0-07}
\end{minipage}
...
\Q A l'état totalement chargé, la structure du composé est \ce{LiC6}. Écrire la demi-équation redox à cette électrode lors de la charge et de la décharge et préciser le signe de l'électrode de graphite.
