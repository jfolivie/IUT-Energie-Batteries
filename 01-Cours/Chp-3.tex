...
Nous avons montré que dans une pile où l'on étudie la tension à courant nul en fonction d'une référence $| \ce{Li^+} (\text{électrolyte}) | \ce{Li (s)} ~ \ominus$, l'enthalpie de réaction du pôle $\oplus$ est donné par : 
\begin{equation*}
...
\setlength{\columnsep}{5pt}
\begin{wrapfigure}{r}[10pt]{0cm}% r à droite et l pour à gauche
...
Comme représenté dans la Fig. \ref{fig:03-04c}, l'abaissement de la $\sigma^\star$ du métal se traduit par un abaissement de l'énergie électronique à la cathode. 

\begin{minipage}{0.5\textwidth}
\centering
\includegraphics[width=\textwidth]{Fig04c}
\end{minipage}\hfill\begin{minipage}{0.45\textwidth}
\captionof{figure}{Abaissement de l'énergie électronique au pôle $\oplus$.}
\label{fig:03-04c}
\end{minipage}
...
Dans les structures présentant une architecture moléculaire\mySideNote{olivine et spinelle} peu distordue par la présence ou l'absence des ions \ce{Li+}, l'énergie potentielle du Li à la cathode est quasi constant à toute composition.
...
Par conséquent, lorsque la batterie est en charge, modélisée par l'équation de demi-réaction au pôle $\oplus$ :
\begin{align*}
...
&= \frac{1}{2} D^\circ (\ce{O=O}) - 2 D^\circ ( \ce{M-O} )
\end{align*}
\end{minipage}\hfill \begin{minipage}{0.45\textwidth}
\begin{tabular}{cc}
\toprule
...
Si on calcule l'énergie thermique reçue par mole d'avancement ($= \Delta_r H^\circ$), on trouve \SI{-155}{\kilo\joule\per\mole}, soit $-62 R T_{amb}$ ! Inéluctablement, toute augmentation du potentiel cathodique au délà de 4.2-4.3 \si{\volt} risque de conduire à l'explosion de la structure lamellaire via une réaction exothermique.
...
Cependant, lors de leur synthèse, les composés aux \ce{Ni} ont tendance à s'éloigner fortement de la st{\oe}chiométrie.
...
En effet, si on reprend les rayons ioniques présentés à la Fig. \ref{fig:03-07}, on observe que le \ce{Ni^{2+}} a un rayon ionique proche de celui du \ce{Li^+}. Par conséquent, comme présenté à la Fig. \ref{fig:03-11}, la non-st{\oe}chiométrie est provoquée par le \blancbis{déplacement} d'ions \ce{Ni^{2+}} au sein des sites \blancbis{octaédriques} des galeries\mySideNote{il s'agit d'un défaut de Frenkel}, ce qui se traduit par l'équation de réaction : 
...
Bien entendu, comme illustré à la Fig. \ref{fig:03-10b}, un tel écart à la st{\oe}chiométrie se traduit par une \blancbis{dégradation} notable des capacités spécifiques expérimentales du dioxyde de nickel. De plus, la diffusion du \ce{Li^+} est ralentie \blancbis{difficile} au sein des galeries.

Dans ce cas, un chimiste ne peut que pousser le système vers la st{\oe}chiométrie. Pour cela, comme on s'y attend, la synthèse de \ce{LiNiO2} dans des conditions très oxydantes et à une température bien choisie permettra sûrement d'approcher au mieux la st{\oe}chiométrie.\mySideNote{on est parvenu à synthétiser tout de même un composé comme \ce{Li Ni_{1.02} O_2} !}
...
Il se trouve que l'ajout de manganèse a permis le développement du matériau \ce{LiCo_{1/3}Ni_{1/3}Mn_{1/3}O2}, appelé \blancbis{NMC} par les spécialistes du domaine, qui est un des meilleurs compromis entre stabilité en cyclage, capacité de décharge et stabilité thermique. 
...
\section{Bilan}
Nous avons vu que le chimiste sur une bonne lecture du tableau périodique et une connaissance des matériaux simples (les \ce{LiMO2}) est capable d'orienter ses substitutions afin d'aboutir à un matériau qui réponde au mieux au cahier des charges : \blancbis{capacité}, \blancbis{stabilité électrochimique} \& \blancbis{stabilité thermique}.  
...
Bien entendu, toute mesure des performances des matériaux d'électrode se fait par une bonne maîtrise de la lecture des \blancbis{courbes de cyclage} des matériaux d'électrode. 
