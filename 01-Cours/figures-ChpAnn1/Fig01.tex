...
\path \practica {1}{Variables}{position $\vec{OM}$, vitesse $\vec{v}$};
\path (p1.north)+(0.0,2.5) \practica{2}{\'{E}quation d'évolution : lois de Newton}{$m \frac{d \vec{v}}{dt} = \sum_i \vec{F}_{i,ext}$};
\path (p2.north)+(2.5,2.5) \practica{4}{Equilibres mécaniques}{$\left( \frac{d}{dt}\vec{v} \right)_{eq} = 0$};
\path (p2.north)+(-2.5,2.5) \practica{3}{Propriétés}{énergie mécanique $E_m$, énergie cinétique $E_c$$\ldots$};
...
\background{p3}{p4}{p4}{p1}{\hspace{0.4cm}Système mécanique}
